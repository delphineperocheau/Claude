...
for i, x in enumerate(P['items']):
    k, d, lab = x['f'], x['d'], x['lab']
    m = M[k]; s = x['start']; sid = f"s{i}"
...
e0 = P['end_start']; total = P['total']; END = round(total - e0, 3)
...
for j, (s, lab) in enumerate(chapters):
    ...
    s2 = round(s + (3.3 if s == 0 else 0.15), 3); d = round(en - s2, 3)
...
<html lang="fr" data-resolution="portrait">
  <head>
    ...
    <title>Far West 360° WeRoad 2</title>
